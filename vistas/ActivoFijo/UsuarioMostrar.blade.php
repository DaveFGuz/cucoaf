...
function envia(){
   window.location="http://localhost:8081/cucoaf/vistas/ActivoFijo/UsuarioIngresa.blade.php";
  }
 function sele(){
  var cond= $("#condi").val();
  if (cond==1) {
     window.location="http://localhost:8081/cucoaf/vistas/ActivoFijo/UsuarioMostrar.blade.php";
  }else{window.location="http://localhost:8081/cucoaf/vistas/ActivoFijo/UsuarioInactivo.blade.php";}
...
//activa el activo C:\xampp\htdocs\cucoaf\vistas\ActivoFijo
$est=1;
$var=$_GET['btnalta1'];
$sql = " UPDATE usuarios set estado='$est' WHERE idusu='$var'";
...
<button type="button"  class="btn btn-primary " onclick="envia()">Registrar nuevo usuario</button>
</div>
</div>
...
<thead>
    ...
    <th scope="col" style="color:#FFFFFF">No. de DUI</th>
    <th scope="col" style="color:#FFFFFF">Telèfono</th>
      <th scope="col" style="color:#FFFFFF">Nombre de usuario</th>
   <th scope="col" style="color:#FFFFFF">Nivel</th>
    <th  style="color:#FFFFFF" WIDTH="100" HEIGHT='9'>Opciones</th>
  </tr>
  </thead>
  <tbody  class="contenidobusqueda">
  <?php
$extraer="SELECT * FROM usuarios";
...
while($ejecuta=mysqli_fetch_array($ejecutar))
{
  ...
  <tr>
    ...
    <td> <?php echo $ejecuta['dui']?></td>
    <td> <?php echo $ejecuta['telefono']?></td>
    <td> <?php echo $ejecuta['user']?></td>
    <td> <?php echo $ejecuta['nivel']?></td>
    
    <td>
     <form  action="UsuarioEditar.blade.php" method="post" class="form-register" > 
      <button  type="submit" class="btn btn-danger" id="btnEditar" name="btnEditar" style="background-color: transparent border:0" data-toggle="modal"  value="<?php echo $ejecuta['idusu']?>" >Editar</button>
    </form>
      
     <form style="margin-left: 70px; margin-top:-33px;" action="UsuarioInactivo.blade.php" method="get" class="form-register" > 
     <button  type="submit" class="btn btn-warning" id="btnbaja" name="btnbaja" style="background-color: transparent border:0" data-toggle="modal"  value=<?php echo $ejecuta['idusu'] ?>>Baja</button>
     </form>
    </td>
  </tr>
...
echo "<script type='text/javascript'>";
        echo "mostrarMensaje('Datos almacenados correctamente!!!', 'success',2500);";
        echo "</script>";
...
echo "<script type='text/javascript'>";
        echo "mostrarMensaje('Datos editados correctamente!!!', 'success',2500);";
        echo "</script>";
